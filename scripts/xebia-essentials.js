let xebia_essential = {
  ...
  cards: [
    ...
    {
      href: "/fallacies-distributed-computing/",
...
function show_new_card() {
  chrome.storage.sync.get({ "xebiaStorageKey": DEFAULT_XEBIA_STORAGE }).then((result) => {
    ...
    let card_url = xebia_essential.domain + card.href;
    chrome.tabs.create({ url: card_url });
    ...
    post_on_slack(`Xebia card of the day: <${card_url}|${card.title}>.`);
  });
}

function reset() {
  chrome.storage.sync.set({
    xebiaStorageKey: DEFAULT_XEBIA_STORAGE
  }).then(() => {
  });
}
...
function post_on_slack(message) {
  chrome.storage.sync.get({ "xebiaStorageKey": DEFAULT_XEBIA_STORAGE }).then((result) => {
    let xebiaState = result.xebiaStorageKey;
    if (xebiaState.slack.channel && xebiaState.slack.token) {
      (async () => {
        let SLACK_URL = `https://crop-r.slack.com/services/hooks/incoming-webhook?token=${xebiaState.slack.token}`;
        let data = {
          "channel": xebiaState.slack.channel,
          "text": message,
          "attachments": []
        };
